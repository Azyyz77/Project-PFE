...
export default function ClientDocumentsPage() {
  ...
  const getCategoryColor = (category: string) => {
    const colors: Record<string, { bg: string; text: string; icon: any }> = {
      ...
      'SAV': { bg: 'bg-red-100', text: 'text-red-700', icon: Layers },
      'Manuel': { bg: 'bg-purple-100', text: 'text-purple-700', icon: FileText },
    };
    return colors[category] || { bg: 'bg-slate-100', text: 'text-slate-700', icon: FileText };
  };
    ...
    <ClientPageWrapper className="space-y-12 pb-20">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="max-w-2xl text-center md:text-left">
            ...
            <h1 className="mb-4 text-4xl sm:text-6xl font-black tracking-tight leading-none">
              Vos <span className="text-red-500">Fichiers</span>
            </h1>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-6 px-4">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-xl font-black text-slate-800 tracking-tight">Filtrer par catégorie</h2>
        </div>
            ...
            <button
              ...
              className={`px-6 py-2.5 rounded-full text-[10px] font-black uppercase tracking-widest transition-all duration-300 ${
                selectedCategory === cat
                  ...
                  : 'bg-white text-slate-400 hover:text-slate-600 shadow-sm border border-slate-100 hover:border-slate-200'
              }`}
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredDocuments.map((doc, idx) => {
              ...
              return (
                ...
                >
                  <ClientCard className="group h-full flex flex-col p-8 border-none shadow-xl shadow-slate-100 hover:shadow-2xl hover:shadow-blue-500/10 transition-all duration-500">
                    <div className="flex items-start justify-between mb-6">
                      <div className={`flex h-14 w-14 items-center justify-center rounded-2xl ${category.bg} ${category.text} shadow-inner group-hover:scale-110 transition-transform`}>
                        <CategoryIcon className="h-7 w-7" />
                      </div>
                      <Badge className={`${category.bg} ${category.text} border-none px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest`}>
                        {doc.categorie}
                      </Badge>
                    </div>
                      ...
                      <div className="flex items-center justify-between p-4 rounded-2xl bg-slate-50 border border-slate-100">
                         <div className="flex items-center gap-3">
                            ...
                            <div className="space-y-0.5">
                               <p className="text-[9px] font-black text-slate-300 uppercase tracking-widest leading-none">Ajouté le</p>
                               <p className="text-xs font-bold text-slate-600">{new Date(doc.date_creation).toLocaleDateString('fr-FR', { day: '2-digit', month: 'long', year: 'numeric' })}</p>
                            </div>
                         </div>
                         {doc.taille_mo && (
                            <div className="text-right">
                               <p className="text-[9px] font-black text-slate-300 uppercase tracking-widest leading-none mb-1">Taille</p>
                               <span className="inline-flex px-2 py-0.5 rounded-md bg-white text-[10px] font-black text-slate-500 border border-slate-100">
