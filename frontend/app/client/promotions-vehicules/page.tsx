        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-4">
          <div className="max-w-2xl text-center md:text-left">
            ...
            <h1 className="mb-4 text-4xl sm:text-4xl font-bold tracking-tight leading-none">
              Nos <span className="text-blue-500">Véhicules</span> en Promo
            </h1>
        ...
        {marques.length > 0 && (
          <div className="flex flex-wrap gap-3 justify-center md:justify-start">
            <button
              ...
              className={`px-8 py-3 rounded-lg font-bold uppercase tracking-wide text-[10px] transition-all duration-300 ${
                selectedMarque === 'all'
                  ? 'bg-blue-500 text-white shadow-sm shadow-blue-500/20'
                  : 'bg-white text-[#8A8D91] hover:bg-[#F0F2F5] border border-[#E4E6EB] shadow-sm'
              }`}
            >
                ...
                <ClientCard className="h-full flex flex-col p-0 overflow-hidden border-none shadow-sm shadow-slate-200/40 group-hover:shadow-blue-500/10 transition-all duration-500">
                  ...
                  <div className="relative h-64 w-full overflow-hidden">
                    ...
                    {promo.pourcentage_reduction && (
                      <div className="absolute top-6 left-6 bg-blue-500 text-white px-5 py-2 rounded-lg font-bold text-lg shadow-md shadow-red-500/30">
                        -{promo.pourcentage_reduction}%
                      </div>
                    )}
                    ...
                    {promo.stock_disponible !== undefined && promo.stock_disponible !== null && (
                      <div className={`absolute top-6 right-6 px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-wide shadow-lg ${
                        promo.stock_disponible > 0 ? 'bg-emerald-500 text-white' : 'bg-blue-600 text-white'
                      }`}>
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 flex flex-col justify-end p-8 text-white">
                      <p className="text-[10px] font-bold uppercase tracking-wide text-blue-400 mb-2">Offre limitée</p>
                      <h4 className="text-xl font-bold tracking-tight">{promo.marque_nom} {promo.modele_nom}</h4>
                    </div>
                  </div>
                  ...
                  <div className="p-8 flex flex-col flex-1">
                    <div className="flex items-center gap-2 mb-4 text-[#B0B3B8]">
                      ...
                      <span className="text-[10px] font-bold uppercase tracking-wide">Exclusivité STA Chery</span>
                    </div>

                    <h3 className="text-2xl font-bold text-[#050505] tracking-tight mb-2 group-hover:text-blue-600 transition-colors">
                      {promo.titre}
                    </h3>
                    
                    <p className="text-sm font-bold text-[#B0B3B8] uppercase tracking-wide mb-4">
                      {promo.marque_nom} {promo.modele_nom} • {promo.version_nom}
                    </p>

                    {promo.description && (
                      <p className="text-[#8A8D91] text-sm font-medium mb-8 line-clamp-2 leading-relaxed">
                        {promo.description}
                      </p>
                    )}
                    ...
                    <div className="mt-auto space-y-8">
                      {/* Pricing */}
                      <div className="flex items-end justify-between border-b border-[#E4E6EB] pb-6">
                        <div>
                          {promo.prix_original && (
                            <p className="text-sm font-bold text-[#B0B3B8] line-through mb-1">
                        ...
                        <div className="text-right">
                          <p className="text-[10px] font-bold text-[#B0B3B8] uppercase tracking-wide mb-1">Valide jusqu'au</p>
                      ...
                      {/* Agency Info */}
                      {promo.agence_nom && (
                        <div className="flex items-center gap-4 p-5 rounded-lg bg-[#F0F2F5] border border-[#E4E6EB]">
                          <div className="h-12 w-12 rounded-lg bg-white border border-[#E4E6EB] flex items-center justify-center shadow-sm shrink-0">
                            <MapPin className="h-6 w-6 text-blue-500" />
                          </div>
                          <div className="overflow-hidden">
                            <p className="text-[10px] font-bold text-[#B0B3B8] uppercase tracking-wide mb-0.5">Disponible chez</p>
                            <p className="text-sm font-bold text-slate-700 truncate">{promo.agence_nom}</p>
                      ...
                      {promo.conditions && (
                        <div className="flex gap-2 text-[10px] font-bold text-[#B0B3B8] italic">
