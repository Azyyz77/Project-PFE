...
import { Wrench, ArrowRight, Package, Clock, CheckCircle, XCircle, Loader2, Eye, Calendar, MapPin, User, Car, FileText, DollarSign } from 'lucide-react';
...
export default function ClientOrdersPage() {
  ...
  const getStatusBadge = (statut: string) => {
    const statusConfig: Record<string, { label: string; className: string }> = {
      PLANIFIE: { label: 'Planifié', className: 'bg-blue-100 text-blue-800' },
      ...
      TERMINE: { label: 'Terminé', className: 'bg-emerald-100 text-emerald-800' },
  ...
  return (
    <div className="w-full h-full overflow-auto bg-gradient-to-br from-blue-50 to-orange-50">
      <div className="max-w-7xl mx-auto px-4 py-8">
        <h1 className="text-4xl font-bold text-slate-900 mb-8">Mes Commandes de Réparation</h1>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
            ...
            <div className="bg-white rounded-xl p-6 shadow-sm border border-slate-200">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm text-slate-600">Coût total</p>
        ...
        {orders.length === 0 ? (
          <div className="text-center py-20 bg-white rounded-3xl border-2 border-dashed border-slate-300">
            ...
            <p className="text-slate-600 mb-6 text-lg">Aucune commande de réparation</p>
            ...
            {orders.map((order) => (
              <div key={order.id} className="bg-white rounded-xl p-6 shadow-sm border border-slate-200 hover:shadow-md transition-shadow">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4 border-t border-slate-200">
                  <div>
                    <p className="text-xs text-slate-500 mb-1">Agence</p>
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium text-slate-900">{order.nombre_interventions} intervention(s)</p>
                  </div>
                  <div>
                    <p className="text-xs text-slate-500 mb-1">Coût</p>
                    <p className="text-sm font-medium text-slate-900">
                      {order.cout_total ? `${order.cout_total.toFixed(2)} TND` : 'Non défini'}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-6">
                ...
                <div className="bg-slate-50 rounded-lg p-4">
                  <div className="flex items-center justify-between mb-4">
                    <h3 className="text-lg font-semibold">Commande #{selectedOrder.order.id}</h3>
                    {getStatusBadge(selectedOrder.order.statut)}
                  </div>
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      <p className="text-slate-600">Date du rendez-vous</p>
                      <p className="font-medium">{formatDate(selectedOrder.order.date_heure)}</p>
                    </div>
                    <div>
                      <p className="text-slate-600">Durée estimée</p>
                ...
                <div>
                  ...
                  <div className="bg-slate-50 rounded-lg p-4 grid grid-cols-2 gap-3 text-sm">
                    <div>
                      <p className="text-slate-600">Marque/Modèle</p>
                    ...
                    <div>
                      <p className="text-slate-600">Couleur</p>
                      <p className="font-medium">{selectedOrder.order.couleur}</p>
                    </div>
                    <div>
                      <p className="text-slate-600">Année</p>
                ...
                <div>
                  ...
                  <div className="bg-slate-50 rounded-lg p-4 text-sm">
                    ...
                    <p className="text-slate-600">Tél: {selectedOrder.order.agence_telephone}</p>
                  </div>
                </div>
                  ...
                  <div>
                    ...
                    <div className="bg-slate-50 rounded-lg p-4 text-sm">
                      ...
                      {selectedOrder.order.agent_telephone && (
                        <p className="text-slate-600">Tél: {selectedOrder.order.agent_telephone}</p>
                      )}
                    </div>
                  </div>
                )}
                ...
                <div>
                  <h4 className="font-semibold mb-3 flex items-center gap-2">
                    ...
                    Interventions ({selectedOrder.interventions.length})
                  </h4>
                ...
                {selectedOrder.attachments.length > 0 && (
                  <div>
                    <h4 className="font-semibold mb-3 flex items-center gap-2">
                      ...
                      Pièces jointes ({selectedOrder.attachments.length})
                    </h4>
                    <div className="space-y-2">
